...
- [ ] 1.1 Add `score_post_compression_reconstruction(sae, feed,
...
- [ ] 5.1 Add `scripts/forge_pipeline.py --matrix` mode (or
...
- [ ] 5.2 Skip cells where the corresponding SAE checkpoint doesn't
      exist (with a clear log line).
- [ ] 5.3 Resumable: skip cells whose `forge_results.json` already
      exists, unless `--force` is passed.
- [ ] 5.4 Print a final summary table showing
      `(run_id × encoding → post-A, post-B, forge_score)`.

## 6. Update scoreboard overview

- [ ] 6.1 The "Three axes of measurement" intro table near the top of
...
- [ ] 6.2 Verify the recommended-defaults table still cites the right
      evidence after the new columns land.

## 7. Verify

- [ ] 7.1 Run the matrix sweep; confirm 8 cells populated.
- [ ] 7.2 Visually inspect: do any encoding choices show wildly
      different post-A or post-B? (Expected: yes — Rung3 keeps 16
      features, Rung5(n_amp_qubits=2) keeps 32, so the columns should
      differ.)
- [ ] 7.3 If a clear winner-per-axis emerges, update the
      recommended-defaults table to cite the cross-encoding post-A and
      post-B evidence.

## 8. Documentation

- [ ] 8.1 Update the scoreboard section's aside to flag this is the
      first time post-polygram A and B are surfaced; cite the
      provenance.
- [ ] 8.2 Archive this change directory once landed.

## 9. Acceptance gate

- [ ] 9.1 `forge_results.json` includes `post_compress_score` with
      `var_explained`, `coverage_0.95`, `coverage_0.90`,
      `mean_best_auc`, `n_kept`.
- [ ] 9.2 Scoreboard "Forge pipeline runs" table has the four new
      columns and a per-cell delta colour.
- [ ] 9.3 At least 4 (run_id × encoding) cells measured and rendered.
- [ ] 9.4 The post-A / post-B columns produce non-uniform values
